            ...
            {reviews.map((review) => (
              <div key={review.id} className="bg-white rounded-lg shadow-sm overflow-hidden">
                <div className="p-4 border-b">
                  <Link to={`/restaurant/${review.restaurantId}`} className="text-xl font-semibold text-food-600 hover:underline">
                    {review.restaurantName || "Restaurant"}
                  </Link>
                  <p className="text-sm text-gray-500">
                    Reviewed on {new Date(review.datePosted).toLocaleDateString()}
                  </p>
                </div>
